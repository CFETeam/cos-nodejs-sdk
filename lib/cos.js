			...
			res.on('data', function (data) {
				...
				.on('end', function () {
					...
					if (ret) {
						...
						callback({
							'httpcode': res.statusCode,
							'code': COS_NETWORK_ERROR,
							'message': 'response ' + body.toString() + ' is not json',
							'data': {}
						});
					}
				});
		...
		if (isExists && typeof callback === 'function') {
			...
			fsRS.on('end', function () {
				fs.stat(filePath, function (error, stats) {
					...
					headers['Host'] = 'web.file.myqcloud.com';
			...
			callback({
				'httpcode': 0,
				'code': COS_PARAMS_ERROR,
				'message': 'file ' + filePath + ' not exists or params error',
				'data': {}
			});
		}
	});
